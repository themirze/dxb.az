...
export default function LinuxCommands2Page() {
  ...
  const progress = Math.round((solved.length / linuxTasks2.length) * 100);
    ...
    <main className="mx-auto max-w-3xl px-6 py-24 sm:py-32">
      <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">
          Linux Commands 2<span className="text-accent">_</span>
        </h1>
        ...
        <div className="mt-6 max-w-[220px]">
          <div className="flex justify-between text-xs text-white/40">
            ...
            <span>
              {solved.length}/{linuxTasks2.length}
            </span>
          </div>
